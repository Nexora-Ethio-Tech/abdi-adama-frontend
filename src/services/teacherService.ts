...
export const submitCourseGrades = async (courseId: string, submissionType: string, options?: { academicYear?: string; semester?: number }) => {
  const response = await api.post('/teacher/grades/submit-course', { courseId, submissionType, ...options });
  return response.data;
};
